...
class FW_Shortcode_Map extends FW_Shortcode {
		...
		if (empty($this->data)) {
			$this->data = apply_filters('fw_shortcode_map_provider', array(
				'custom' => array(
					...
					'label'      => __('Custom','teadokan'),
					'options'    => array(
						'locations' => array(
							'label' => __('Locations', 'teadokan'),
							'popup-title' => __('Add/Edit Location', 'teadokan'),
							'type' => 'addable-popup',
							'desc' => false,
							'template' => '{{  if (location.location !== "") {  print(location.location)} else { print("' . __('Note: Please set location', 'teadokan') . '")} }}',
							'popup-options' => array(
								'location' => array(
									'type' => 'map',
									'label' =>__('Location','teadokan'),
								),
								'title' => array(
									'type' => 'text',
									'label' => __('Location Title', 'teadokan'),
									'desc' => __('Set location title', 'teadokan'),
								),
								'description' => array(
									'type'  => 'textarea',
									'label' => __('Location Description', 'teadokan'),
									'desc'  => __('Set location description', 'teadokan')
								),
								'url' => array(
									'type'  => 'text',
									'label' => __('Location Url', 'teadokan'),
									'desc'  => __('Set page url (Ex: http://example.com)', 'teadokan'),
								),
								'thumb' => array(
									'label'       => __('Location Image', 'teadokan'),
									'desc'        => __('Add location image', 'teadokan'),
									'type'        => 'upload',
								)
							)
						)
	...
	protected function _render($atts, $content = null, $tag = '')
	{
		if (!isset($atts['data_provider']['population_method'])) {
			trigger_error(
				__('No location provider specified for map shortcode', 'teadokan')
			);
			return '<b>' . __( 'Map Placeholder', 'teadokan' ) . '</b>';
		}
		...
		if (!isset($this->data[$provider])) {
			return '<!-- WARNING: '
			       . sprintf(__('Unknown location provider "%s" specified for map shortcode', 'teadokan'), $provider)
			       . ' -->';
		}
