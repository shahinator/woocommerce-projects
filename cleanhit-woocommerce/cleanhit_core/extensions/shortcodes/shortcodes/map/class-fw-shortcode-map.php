...
class FW_Shortcode_Map extends FW_Shortcode {
		...
		if (empty($this->data)) {
			$this->data = apply_filters('fw_shortcode_map_provider', array(
				'custom' => array(
					...
					'label'      => __('Custom','cleanhit'),
					'options'    => array(
						'locations' => array(
							'label' => __('Locations', 'cleanhit'),
							'popup-title' => __('Add/Edit Location', 'cleanhit'),
							'type' => 'addable-popup',
							'desc' => false,
							'template' => '{{  if (location.location !== "") {  print(location.location)} else { print("' . __('Note: Please set location', 'cleanhit') . '")} }}',
							'popup-options' => array(
								'location' => array(
									'type' => 'map',
									'label' =>__('Location','cleanhit'),
								),
								'title' => array(
									'type' => 'text',
									'label' => __('Location Title', 'cleanhit'),
									'desc' => __('Set location title', 'cleanhit'),
								),
								'description' => array(
									'type'  => 'textarea',
									'label' => __('Location Description', 'cleanhit'),
									'desc'  => __('Set location description', 'cleanhit')
								),
								'url' => array(
									'type'  => 'text',
									'label' => __('Location Url', 'cleanhit'),
									'desc'  => __('Set page url (Ex: http://example.com)', 'cleanhit'),
								),
								'thumb' => array(
									'label'       => __('Location Image', 'cleanhit'),
									'desc'        => __('Add location image', 'cleanhit'),
									'type'        => 'upload',
								)
							)
						)
	...
	protected function _render($atts, $content = null, $tag = '')
	{
		if (!isset($atts['data_provider']['population_method'])) {
			trigger_error(
				__('No location provider specified for map shortcode', 'cleanhit')
			);
			return '<b>' . __( 'Map Placeholder', 'cleanhit' ) . '</b>';
		}
		...
		if (!isset($this->data[$provider])) {
			return '<!-- WARNING: '
			       . sprintf(__('Unknown location provider "%s" specified for map shortcode', 'cleanhit'), $provider)
			       . ' -->';
		}
